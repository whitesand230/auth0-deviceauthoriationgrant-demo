...
const AUTHZ_ENDPOINT = 'https://' + process.env.AUTH0_HOST + '/oauth2/' + AUTHZID + '/v1/device/authorize';
const TOKEN_ENDPOINT = 'https://' + process.env.AUTH0_HOST + '/oauth/token';
const CODE_ENDPOINT = 'https://' + process.env.AUTH0_HOST + '/oauth/device/code';
...
app.get('/', (req, res) => {
    ...
    if (req.cookies.resdata != null || req.cookies.resdata == '') {
        ...
        axios.post(CODE_ENDPOINT, qs.stringify(payload), { headers })
            .then(response => {
...
function displayPage(responseData, res, addCookie){
    QRCode.toDataURL(responseData.verification_uri_complete, function (err, url) {
        if (err) return console.log("error occurred")
        if(process.env.DEBUG === 'true'){
            console.log("***Auth0 Authorization Endpoint response***");
            console.log("Response from Authorization Endpoint: ")
            console.log(responseData);
        }
        if(addCookie) {
            res.cookie("resdata", responseData, {'sameSite': 'strict'});
        }
        var modifyString = responseData.user_code;
        res.render('pages/index', {
            jsonResData: JSON.stringify(responseData),
            welcomeLine: 'Please visit <a href="' + responseData.verification_uri + '" target="_blank">' + responseData.verification_uri + '</a> and enter code: <br/> <b>' + modifyString + '</b> to activate your device.',
            qrCode: url,
